> **СТАТУС: исторический анализ** (входная точка — состояние после PLAN-005, 2026-09-11). Реализованные фазы и текущее состояние — [CURRENT.md](CURRENT.md); выбор следующей фазы — через PLAN-020-AUDIT §WHAT REMAINS и отдельное решение владельца.

# NEXT-PHASE — сопоставительный анализ следующей фазы разработки

STATUS: DECISION ANALYSIS — это **не** план
Дата: 2026-09-11
Вход: [VISION](../VISION.md) + [PRODUCT-ARCHITECTURE](../PRODUCT-ARCHITECTURE.md) +
[PRODUCT-MODEL](../PRODUCT-MODEL.md) + [PRODUCT-SURFACE-MAP](../PRODUCT-SURFACE-MAP.md) +
[DAILY-EXPERIENCE](../DAILY-EXPERIENCE.md) (целевое состояние) •
[PROJECT](../PROJECT.md) + [CURRENT](CURRENT.md) + код `mta-market-site` (фактическое состояние)

---

## 0. Назначение

[DAILY-EXPERIENCE §50](../DAILY-EXPERIENCE.md) фиксирует: после фиксации документа
план НЕ создаётся автоматически. Сначала нужно сопоставить пять foundational-документов
и определить, какая **минимальная следующая development phase** создаст наибольший рост
реальной ценности платформы. Этот документ — результат такого сопоставления.

Development Plan (PLAN-006) из него **не создаётся**: план фиксируется отдельным
решением и оформляется в [DEVELOPMENT/ACTIVE/](ACTIVE/) по правилам [README](README.md).

---

## 1. Фактическое состояние (после PLAN-005)

Сопоставление требований DAILY-EXPERIENCE с тем, что реально существует
(проверено по коду `mta-market-site`, см. также CURRENT):

| Элемент daily experience | Есть (факт) | Нет (гэп) |
|---|---|---|
| Источники активности | Server (status/news/updates), Resource (release/версии), Forum (темы/ответы), Review (серверы/ресурсы) | Creator publications (статей нет — Content pillar пуст) |
| LIVE-слой | Мониторинг по каждому серверу: ONLINE/OFFLINE/UNKNOWN из реальных heartbeat-сэмплов | Глобальных агрегатов «N игроков / M серверов онлайн» не существует |
| Живая Home | Home — маркетплейс-лендинг (Новинки / Популярное / Бесплатные через `/resources/homepage`) | Снапшот «что происходит»: live line, смешанная активность, популярное по типам |
| Continuous feed | `/news` (только server news/updates), activity-блок на `/community` (только форум) | Единого read-слоя агрегированной активности нет; маршрута `/activity` не существует |
| Follow | Server follow + счётчик + уведомления | Creator/Resource/Thread/Community follow (осознанно позже — §16) |
| Notifications | In-app: SERVER_NEWS / SERVER_UPDATE / FORUM_REPLY / REVIEW_EVENT / MODERATION, deep links, колокольчик | Email/push (осознанно позже — §46) |
| Dashboard | «My MTA» виджеты: мои серверы, подписки, обсуждения, уведомления | Сводки «Сейчас / За ночь» (§5, §15) |
| Privacy | Privacy by default в server/review/purchase (PLAN-005, проверено E2E) | — |

**Главный вывод:** задняя половина return-петли (FOLLOW → NOTIFICATION → RETURN)
уже реализована в PLAN-005. Передняя половина — «открыл сайт и за 30 секунд увидел
жизнь экосистемы» (DAILY-EXPERIENCE §2, §47) — отсутствует: Home отвечает на вопрос
«что можно купить?», а не «что происходит в MTA?». Все шесть источников активности
существуют как данные, но нет ни одной поверхности, где они сходятся.

---

## 2. Сопоставление foundational-документов

- **VISION** (§3 North Star, §57 North Star Metric, §63 Final Vision): «место,
  которое человек открывает, когда хочет узнать, что происходит в MTA:SA», метрика —
  Weekly Active Community Members. DAILY-EXPERIENCE — прямая операционализация
  North Star: return loop и есть механизм weekly participation.
- **PRODUCT-ARCHITECTURE** (§38 Home, §39 Daily Return Loop, §29 actionable
  notifications, §31 follow): Home «должна отражать текущую жизнь платформы» —
  сегодня не отражает; Daily Return Loop описан архитектурно, но не реализован
  как поверхность.
- **PRODUCT-MODEL** (§41 Discovery, §43 Daily Platform Loop, §31 Notification):
  «Login → sees updates → opens … → follows → notification → returns» — самый
  первый шаг («sees updates») вне dashboard не существует.
- **PRODUCT-SURFACE-MAP** (§3 Home blocks: current community activity, server
  highlights, latest news, latest discussions, creators; §33 My Dashboard):
  целевые блоки Home описаны, но ни один «живой» блок не реализован.
- **DAILY-EXPERIENCE** добавляет недостающий слой правил: live layer (§4),
  activity — только derived read layer, не новая доменная сущность (§17, §45),
  минимальный набор activity types (§18), chronological + deterministic ranking
  без ML (§19), приоритет high-value событий (§40), privacy (§41–42),
  performance-бюджет (§44).

Противоречий между документами нет. Отмечено одно расширение поверхности:
потенциальный маршрут `/activity` (DAILY-EXPERIENCE §21) не входит в
SURFACE-MAP §62 (future surfaces) — при фиксации PLAN-006 его стоит добавить
плановым изменением SURFACE-MAP; сам анализ этого не делает.

---

## 3. Кандидаты на следующую фазу

Критерии оценки (по §50): рост **реальной** ценности (причина возвращаться),
опора на существующее, стоимость/риск, зависимость от других фаз.

| Кандидат | Рост ценности | Опора на существующее | Стоимость | Примечание |
|---|---|---|---|---|
| **A. Daily Experience Foundation** | Максимальный: превращает 6 существующих источников в причину возвращаться | Максимальная: read-layer поверх готовых доменов | Низкая/средняя: несколько bounded queries + пересборка Home | Закрывает главный гэп North Star |
| B. Content layer (статьи) | Средний: новый источник, но аудитории чтения ещё нет | Средняя: новый домен целиком | Высокая | Статьям некуда «прийти» без живого Home |
| C. Production verification | Не растит продуктовую ценность, разблокирует боевой запуск | — | Средняя | Необходим до релиза, ортогонален остальным |
| D. Guarantee / Deals | Высокий, но в Trust pillar | Низкая: новый финансовый lifecycle | Очень высокая | Требует доверия и аудитории, которые создаёт петля возврата |
| E. Графики статистики серверов (24h/7d/30d) | Низкий: углубление существующего | Высокая | Низкая | Улучшение, а не петля |
| F. Email/push канал | Средний: усиливает заднюю половину петли | Высокая | Средняя | §27 прямо требует, чтобы возврат работал и без уведомлений |

---

## 4. Рекомендация: минимальная следующая фаза

**A. «Daily Experience Foundation» — кандидат PLAN-006.**

Переход состояния: из «Home = витрина маркетплейса» в «Home = живой вход
в экосистему; пользователь за 30 секунд видит, что происходит в MTA»
(DAILY-EXPERIENCE §2, §47).

Минимальный состав — всё поверх существующих доменов, без новых source of truth:

1. **Глобальные LIVE-агрегаты**: «N игроков онлайн / M серверов онлайн» из
   существующих heartbeat-сэмплов (реальные данные — ARCHITECTURE §63,
   VISION Principle 5; никакого fake online). Кэш с коротким TTL.
2. **Home rebuild** по DAILY-EXPERIENCE §3: блок «Сейчас в MTA» (live line),
   «Активность» (последние высокоценные события), «Популярное» (top servers по
   онлайн, активные обсуждения, свежие ресурсы). Полностью доступно Guest (§29) —
   discover → value → register, SURFACE-MAP §37.
3. **Derived activity read-layer** (§17, §45): единый read-эндпоинт, агрегирующий
   bounded-запросы из существующих таблиц (server updates/news, публикации
   ресурсов/версий, темы/ответы форума, отзывы, новые серверы); окно 7–30 дней,
   индексы, кэш. Не новая доменная сущность и не источник истины.
4. **Типы активности** — минимальный набор §18: SERVER_ONLINE, SERVER_UPDATE,
   SERVER_NEWS, NEW_SERVER, RESOURCE_RELEASE, RESOURCE_UPDATE, NEW_DISCUSSION,
   DISCUSSION_REPLY, NEW_REVIEW. CREATOR_PUBLICATION и NEW_ARTICLE
   зарезервированы под Content-фазу (сейчас не генерируются).
5. **Ранжирование** — chronological + deterministic приоритизация (§19):
   high-value события (updates, releases, статьи) выше реакций и просмотров (§40).
   Без ML, без fake trending (§20).
6. **Dashboard «Сейчас / За ночь»** (§5, §15): сводка «с момента последнего визита»
   по типам — углубление существующих виджетов My MTA, а не новая страница.

**Явно вне scope фазы** (осознанные решения самого DAILY-EXPERIENCE):

- `/activity` как отдельный непрерывный маршрут (§21: Home и Feed не обязаны
  совпадать) — read-layer проектируется так, чтобы маршрут добавлялся позже дёшево;
- articles / creator publications — отдельная Content-фаза;
- follow для creator/resource/thread/community (§16: порядок Server → Creator → …);
- email/push, digest, «Today in MTA» (§23), personalized feed, recommendations,
  trending (§46);
- time-based personalization (§6: сначала только реальная активность);
- изменения notification system (§28 deep links уже реализованы в PLAN-005).

---

## 5. Почему не другие кандидаты

- **B (Content)**: добавляет источник ценности, но DAILY-EXPERIENCE §11 делает
  контент частью ecosystem discovery — слой discovery должен существовать раньше,
  иначе статьи не получают вход и аудиторию. NEW_ARTICLE/CREATOR_PUBLICATION
  в §18 зарезервированы — Content-фаза ляжет на готовый activity layer.
- **C (Production verification)**: обязателен до боевого запуска (blockers
  PLAN-004/005), но это операционная фаза, а не рост продуктовой ценности;
  не конфликтует с A и может идти параллельно как ops-работа.
- **D (Guarantee/Deals)**: самый тяжёлый контур (деньги, споры, юридические
  ограничения — MODEL §25) и он нуждается в доверии и критической массе
  пользователей, которые как раз создаёт петля возврата.
- **E (статистика серверов)**: улучшает существующую страницу сервера,
  не создаёт новых причин возвращаться.
- **F (email/push)**: усиливает уже работающую заднюю половину петли, но §27
  требует, чтобы возврат работал и без уведомлений — сначала фронт петли.

---

## 6. Ограничения и риски

- **Cold start (§38)**: при малой активности снапшот должен иметь честные empty
  states и не показывать пустые блоки ради заполнения (SURFACE-MAP §33).
  Platform health staged — фаза не должна имитировать жизнь.
- **Шум (§40)**: в активность попадают только публичные высокоценные события;
  «someone liked something» не генерирует item.
- **Privacy (§41–42, MODEL §40, ARCHITECTURE §34)**: активность строится только
  из публичных событий; покупки, приватные подписки, follower identities и
  server membership никогда не агрегируются. Связь Server↔Resource — только
  по явному opt-in владельца.
- **Performance (§44, SURFACE-MAP §52)**: bounded queries, временное окно,
  индексы, кэш; Home не должна деградировать от живого контента.
- **Ranking**: Latest/Popular/Trending вводятся только при наличии реальной
  метрики (§19–20) — на первом этапе только хронология + детерминированные
  приоритеты типов.

---

## 7. Статус решения

Анализ зафиксирован как результат сопоставления пяти foundational-документов
(DAILY-EXPERIENCE §50 выполнен). Рекомендованная минимальная фаза —
**Daily Experience Foundation** (кандидат PLAN-006). План создаётся отдельным
решением по правилам Development Plan system; нумерация продолжается с
PLAN-006 (VISION §60). CURRENT.md обновлён на этот анализ.

**Обновление (2026-09-11):** решение принято —
[PLAN-006](COMPLETED/PLAN-006.md) зарегистрирован в `DEVELOPMENT/ACTIVE/`
на основе этого анализа и **выполнен** в тот же день (IMPLEMENTATION
COMPLETE, см. EXECUTION RECORD в конце файла).

---

## 8. ЦИКЛ 2 (2026-09-11): следующая фаза после PLAN-006

PLAN-006 выполнен (daily experience работает). Повторное сопоставление
пяти foundational-документов + факта:

- **CONTENT pillar пуст** — единственный pillar без реализации (TRUST частично:
  reviews/verification есть; deals нет). VISION §15–16: статьи объясняют,
  обучают, связывают пользователей с остальными частями платформы.
- **Activity layer готов к статьям**: DAILY-EXPERIENCE §18 резервирует
  NEW_ARTICLE/CREATOR_PUBLICATION; план PLAN-006 зарезервировал их «под
  Content-фазу» (D-001) — обоснование в §5 (Цикл 1): «Content-фаза ляжет на
  готовый activity layer».
- **Content Loop** (DAILY-EXPERIENCE §11): Article → Reader → Discussion →
  Creator → Resource/Server → Follow → Return — петля начинается только при
  наличии статей.
- **Оголодавший discovery**: живой Home нуждается в источниках (§38 platform
  health: no content → no activity).

Оценка кандидатов (критерии §50):

| Кандидат | Вердикт |
|---|---|
| **Content Foundation (статьи)** | **Выбран.** Новый источник высокоценных событий, готовый слот NEW_ARTICLE, заполняет пустой pillar, умеренная стоимость на готовой инфраструктуре (модерация, media, форум-связи, профили — всё существует) |
| Production verification | Не выбирается агентом: требует инфраструктурных решений владельца (домен, DNS, боевой YooKassa, хостинг); остаётся blocker'ом launch'а, не конкурентом продуктового цикла |
| Графики статистики серверов | Улучшение существующей страницы, не новая петля |
| Events | Тяжёлый новый домен без аудитории |
| Email/push | Усиливает заднюю половину петли; §27 требует обратного приоритета |

**Решение: PLAN-007 — Content Foundation.** План зарегистрирован в
`DEVELOPMENT/ACTIVE/PLAN-007.md` и **выполнен** в тот же день
(IMPLEMENTATION COMPLETE, EXECUTION RECORD в конце файла). Переход состояния:
из «статей не существует» в «статьи — полноценная публичная сущность с
модерацией, обсуждениями, связями с ресурсами/серверами и слотом NEW_ARTICLE
в daily experience».

---

## 9. ЦИКЛ 3 (2026-09-11): следующая фаза после PLAN-007

Сопоставление после PLAN-007 (CONTENT pillar жив, /search построен):

- **Follow model** (DAILY-EXPERIENCE §16): порядок реализации — Server ✅
  (PLAN-005) → **Creator** → **Resource** → Thread → Community. Следующие два
  шага порядка — Creator и Resource — не реализованы, при этом вся задняя
  инфраструктура петли (уведомления с deep links, сводка «Сейчас / За ночь»)
  уже работает.
- **Creator Loop** (§9) и **Resource Following** (§26) — петли, которые
  заканчиваются словами «Return», но без follow у создателя нет возврата.
- **§25**: «Creator → new resource → update → publication. Followers receive
  relevant notifications» — прямой слот для этого цикла.
- **§26**: покупка сама создаёт relationship — уведомление покупателю об
  обновлении купленного ресурса является частью Market Loop (§12), не follow.
- **Privacy** (§42): follower identities никогда не публикуются — только
  агрегаты.
- **Activity layer** не расширяется: публикации создателей уже видны как
  RESOURCE_RELEASE/NEW_ARTICLE; follow — private relationship (§42), не
  событие ленты. CREATOR_PUBLICATION остаётся зарезервированным (devlogs).

Оценка кандидатов:

| Кандидат | Вердикт |
|---|---|
| **Follow Expansion (Creator + Resource)** | **Выбран.** Замыкает порядок §16, усиливает ВСЕ существующие поверхности (storefront, карточка ресурса, статьи) без новых сущностей контента; максимальный рост петли возврата на минимальной стоимости |
| Creator publications / devlogs | Новый контент-тип; усиливает storefront, но требует editor-инфраструктуру; лучше после follow (у devlog'ов будет аудитория) |
| Гарантии/deals (Trust) | Самый тяжёлый контур (платежи/споры/юр.), требует массы доверия; отложить |
| Production verification | Решение владельца инфраструктуры (как в Цикле 2) |
| Графики статистики / events / email/push | Как в Цикле 2: улучшения и каналы, не петли |

**Решение: PLAN-008 — Follow Expansion (Creator + Resource).** Зарегистрирован
и **выполнен** в тот же день (IMPLEMENTATION COMPLETE). Переход состояния:
из «follow есть только у серверов» в «пользователь может следить за
создателем и за конкретным ресурсом и получать уведомления о релизах,
обновлениях и статьях; покупатель уведомляется об обновлении купленного».
Попутно починен разрыв Market Loop: новая версия опубликованного ресурса
теперь проходит re-moderation и выходит с уведомлениями покупателям и
подписчикам.

---

## 10. ЦИКЛ 4 (2026-09-11): следующая фаза после PLAN-008

Сопоставление после PLAN-008 (follow: Server/Creator/Resource; Update-петля
починена):

- **§16 порядок**: Server ✅ → Creator ✅ → Resource ✅ → **Thread** →
  Community. Следующий шаг порядка — Thread.
- **Community Loop** (DAILY-EXPERIENCE §10): Read → Discussion → Reply →
  Reaction → **Follow** → Notification → Return. Шаг Follow в сообществе
  сейчас невозможен: подписаться можно только на сервер; следить за чужим
  обсуждением нельзя. Форум-получатели FORUM_REPLY — только автор темы и
  участники (кто уже ответил), а «молчаливый читатель» (§10: Read) остаётся
  без возвратного канала.
- **Факт-проверка альтернатив**: графики статистики серверов — API-серии
  уже существуют (PLAN-005), на странице сервера сейчас числа без графика
  (кандидат отдельного UX-цикла, не петля); гарантии/deals — заблокированы
  на решения владельца (payouts/юр. модель, MODEL §25 «не банк»);
  production verification — как в Циклах 2–3.
- **Privacy** (§42): как всегда — только агрегаты, никаких списков.
- **Activity layer** не расширяется (следить = приватная relationship).

Оценка кандидатов:

| Кандидат | Вердикт |
|---|---|
| **Thread Follow (Community Loop)** | **Выбран.** Завершает шаг §16 и петлю §10; минимальная стоимость на готовой инфраструктуре (уведомления, dashboard-ряды); усиливает pillar COMMUNITY |
| Графики статистики серверов | API есть, UI-графика нет — UX-улучшение серверной страницы; отложить отдельным UX-циклом |
| Creator devlogs | Дублирует статьи (PLAN-007); CREATOR_PUBLICATION остаётся зарезервированным |
| Гарантии/deals | Заблокировано решениями владельца (payouts, юр. модель удержания средств) |
| Events / email/push / production verification | Как в Циклах 2–3 |

**Решение: PLAN-009 — Thread Follow (Community Loop completion).**
Зарегистрирован и **выполнен** в тот же день (IMPLEMENTATION COMPLETE).
Переход состояния: из «следить можно только за сервером/создателем/ресурсом»
в «пользователь следит за обсуждением и получает уведомление о каждом
ответе, не участвуя в нём; сводка показывает ответы в отслеживаемых темах».

---

## 11. ЦИКЛ 5 (2026-09-11): следующая фаза после PLAN-009

Факт-проверка кандидатов перед выбором:

- **Графики статистики серверов** — снят аудитом: Live-вкладка сервера уже
  показывает 24ч-график (hand-rolled SVG), peak/average/uptime, showStats-
  gate (PLAN-005). Остаётся косметика (7d/30d переключатель) — не фаза.
- **Гарантии/deals + payouts** — по-прежнему заблокированы владельческой
  платёжно-правовой моделью (MODEL §25 «MTA Market не является банком»).
- **Production verification** — решение владельца инфраструктуры.
- **Creator publications/devlogs** — дублирует статьи (PLAN-007);
  CREATOR_PUBLICATION остаётся зарезервированным.
- **Creator analytics** (IDEAS §4: «Seller analytics уровня ecommerce:
  просмотры → конверсия») — **не существует вообще**: у создателя нет ни
  одного числа о своей аудитории. VISION §32 («Зачем мне публиковаться?» →
  audience) и CREATOR Loop (§9) предполагают обратную связь о спросе.

Оценка:

| Кандидат | Вердикт |
|---|---|
| **Creator Analytics Foundation (просмотры ресурсов + аналитика продавца)** | **Выбран.** Новый реальный сигнал (спрос), единственная пустая метрика создателя, поддержка петли продаж; стоимость умеренная (одна агрегат-таблица + эндпоинт + карточка в магазине) |
| Сервер: 7d/30d переключатель + incidents | Косметика готового Live-блока; отложить |
| Wishlist | UX-приятность покупателя; слабее аналитики создателя |
| Deals/payouts, production verification | Владелец |

**Решение: PLAN-010 — Creator Analytics Foundation.** Зарегистрирован и
**выполнен** в тот же день (IMPLEMENTATION COMPLETE). Переход состояния:
из «создатель видит только продажи» в «создатель видит спрос: просмотры
карточек ресурсов (30 дней, по ресурсам), конверсию просмотр→покупка —
только свои данные, без идентичностей зрителей».

## 12. ЦИКЛ 6 (2026-09-12): следующая фаза после PLAN-015

Факт-проверка кандидатов перед выбором:

- **Несколько способов входа и оплаты** (ideas/MARKETPLACE §6 «несколько
  платёжных провайдеров — интерфейс провайдера уже нейтральный»,
  «Пополнение баланса»): оба нейтральных интерфейса (`IIdentityProvider`,
  `IPaymentProvider`) существуют с PLAN-002/E-010 и явно называются
  «готовностью к T-Bank/Alfa/crypto»; вход в платформу ограничен Discord —
  единственная точка отказа входа. Это фаза конфигурации, а не новой доменной
  логики.
- **Задокументированный долг** (PLAN-001…015 записи): мёртвый redirect
  `/account/identities`, admin raw-ID формы, EN targetType в спорах,
  обещанные DESIGN-SYSTEM §10 фокус-трапы, search вне react-query, auth-FOUC,
  7d/30d-переключатель (API готов, UI нет), api-ext монолит, error-страницы,
  footer claims, дрейф документации (TESTING/CURRENT/README/NEXT-PHASE),
  E2E-гигиена (PLAN-006 §17-18) — каждый пункт с источником.
- **Пополнение баланса (top-up)** — владельческая платёжно-правовая модель
  (MODEL §25 «не банк»), по-прежнему не кандидат.
- **Email/push уведомления, password reset** — требуют боевого SMTP-решения
  владельца (PLAN-004 §6).
- **7d-графики серверов** — закрываются в рамках долга D-007 как UI
  переключателя (API готов), полноценная incidents-фаза — позже.

Оценка:

| Кандидат | Вердикт |
|---|---|
| **Identity Expansion + Multi-Provider Payments + Debt Closure** | **Выбран.** Точки входа и оплаты — самые используемые поверхности; оба интерфейса готовы к превращению «готовности» в работающие провайдеры; накопленный долг закрывается тем же планом с источниками каждого пункта |
| Wishlist / favorites | UX-приятность; ниже по приоритету |
| Deals/guarantees, top-up, payouts | Владелец (платёжно-правовая модель) |
| Trust hub, reputation, blacklist | Будущие trust-домены (ideas/TRUST) |
| Production verification, email/push | Владелец инфраструктуры |

**Решение: PLAN-016 — Identity Expansion, Multi-Provider Payments &
Platform Debt Closure.** Зарегистрирован и выполнен 2026-09-12
(IMPLEMENTATION COMPLETE). Переход состояния: «провайдер — это
конфигурация, а не фича» — точка входа и точка оплаты стали реестрами
включённых провайдеров (без env провайдер честно отсутствует), при этом
денежные инварианты не ослаблены (webhook-only completion, re-fetch
сверка, RUB-locked инвойсы, exactly-once поверх существующих механизмов);
задокументированный долг D-002..D-014 закрыт.
