...
describe.skipIf(!dbAvailable)("health endpoints", () => {
...
describe.skipIf(!dbAvailable)("prometheus metrics", () => {
...
describe.skipIf(!dbAvailable)("security headers", () => {
...
describe.skipIf(!dbAvailable)("protocol contract pinning", () => {
...
describe.skipIf(!dbAvailable)("audit log", () => {
...
describe.skipIf(!dbAvailable)("server heartbeat shouldUpdate", () => {
  it("advises an update when a newer PUBLISHED version exists", async () => {
    ...
    const { createServerSigningKey } = await import("@server/lib/drm/service");
