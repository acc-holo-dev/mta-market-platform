...
import { sha256Hex } from "@server/lib/serverIntegration";
...
describe.skipIf(!dbAvailable)("server registration & lifecycle", () => {
  let slug = "";
...
describe.skipIf(!dbAvailable)("server ownership verification via integration heartbeat", () => {
  let slug = "";
  ...
  it("heartbeats that stop arriving degrade to UNKNOWN via sweep (E-006)", async () => {
    const { runMonitoringSweep } = await import("@server/lib/serverMonitoring");
...
describe.skipIf(!dbAvailable)("offline reporting & archived lifecycle", () => {
...
describe.skipIf(!dbAvailable)("privacy switches enforced backend-side", () => {
  let slug = "";
...
describe.skipIf(!dbAvailable)("server follow & follower count", () => {
...
describe.skipIf(!dbAvailable)("server admin moderation with audit trail", () => {
