...
// (global fetch emulation via the shared oauth-harness, same harness as
// auth/identity-link.test.ts). Telegram is tested at the provider level
// only: the Login Widget payload verification is pure and needs no HTTP/DB.
...
const fetchMock = createOAuthFetchStub([
  {
    match: (u) => u.startsWith("https://id.vk.com/oauth2/auth"),
    respond: (_url, init) => {
      tokenCounter += 1;
  ...
  {
    match: (u) => u.startsWith("https://id.vk.com/oauth2/user_info"),
    respond: () => fetchJsonOk(vkUser),
  },
]);
...
const dbAvailable = await probeDatabaseAvailable("[identity-providers.test]");
...
describe.skipIf(!dbAvailable)("VK ID provider (redirect oauth)", () => {
...
describe("Telegram direct login provider", () => {
