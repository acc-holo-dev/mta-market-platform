...
// Same harness style as commerce/payments-webhook-yookassa.test.ts:
...
describe("crypto invoice payment provider adapter", () => {
  ...
  it("verifyWebhook accepts the valid callback sign (parsed and raw-body paths) and rejects tampering", () => {
    ...
    expect(provider.verifyWebhook(providerWebhookCtx(JSON.parse(raw.toString("utf8")), raw))).toEqual({ ok: true });
    ...
    expect(provider.verifyWebhook(providerWebhookCtx(payload))).toEqual({ ok: false, reason: "signature" });
    expect(provider.verifyWebhook(providerWebhookCtx("not-an-object"))).toEqual({ ok: false, reason: "signature" });
  });

  it("parseWebhook routes succeeded/canceled/failed events and nulls unknown payloads", () => {
    const succeeded = provider.parseWebhook(
      providerWebhookCtx({ uuid: "inv-uuid-1", order_id: "order-1", status: "paid", amount: "299.00", paid_amount: "299.00" })
    );
    ...
    expect(provider.parseWebhook(providerWebhookCtx({ uuid: "u1", status: "expire" }))!.eventType).toBe(
      "payment.canceled"
    );
    expect(provider.parseWebhook(providerWebhookCtx({ uuid: "u2", status: "wrong_amount" }))!.eventType).toBe(
      "payment.failed"
    );
    // In-flight statuses stay provider-prefixed (business truth is re-fetched).
    expect(provider.parseWebhook(providerWebhookCtx({ uuid: "u3", status: "confirming" }))!.eventType).toBe(
      "crypto.confirming"
    );

    // Unrecognizable payloads → null (route answers 400).
    expect(provider.parseWebhook(providerWebhookCtx({ status: "paid" }))).toBeNull();
    expect(provider.parseWebhook(providerWebhookCtx({ uuid: "u4" }))).toBeNull();
