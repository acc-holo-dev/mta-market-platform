...
// Harness style mirrors commerce/payments-webhook-yookassa.test.ts:
...
// The literally-common fetch-stub pieces live in the shared harness
// (@tests/tools/helpers/payments-harness).
...
} from "@tests/tools/helpers/payments-harness";
...
describe("T-Bank payment provider adapter", () => {
  ...
  it("getPayment maps CONFIRMED → SUCCEEDED with amount/metadata normalization", async () => {
    ...
    expect(lastFetchBody(fetchMock)).toMatchObject({ TerminalKey: "test-terminal", PaymentId: "123456" });
  ...
  it("verifyWebhook accepts the valid notification token and rejects tampering", () => {
    ...
    expect(provider.verifyWebhook(providerWebhookCtx(body))).toEqual({ ok: true });

    // Tampered field → token mismatch → signature.
    const tampered = { ...body, Amount: 9900 };
    expect(verifyTBankNotification(tampered)).toBe(false);
    expect(provider.verifyWebhook(providerWebhookCtx(tampered))).toEqual({ ok: false, reason: "signature" });

    // Foreign terminal key → auth failure (payload not addressed to us).
    const foreign = { ...body, TerminalKey: "other-terminal" };
    expect(provider.verifyWebhook(providerWebhookCtx(foreign))).toEqual({ ok: false, reason: "auth" });
  });
  ...
  it("verifyWebhook rejects missing/garbage tokens and foreign terminals", () => {
    ...
    expect(
      provider.verifyWebhook(providerWebhookCtx({ TerminalKey: "test-terminal", Amount: 1 }))
    ).toEqual({ ok: false, reason: "signature" });
    // No terminal key at all → payload is not addressed to this merchant.
    expect(provider.verifyWebhook(providerWebhookCtx({ Amount: 1, Token: PINNED_NOTIFICATION_TOKEN }))).toEqual({
      ok: false,
      reason: "auth",
    });
    expect(provider.verifyWebhook(providerWebhookCtx("not-an-object"))).toEqual({
      ok: false,
      reason: "signature",
    });
  });
  ...
  it("parseWebhook routes succeeded/failed/canceled events with stable ids", () => {
    const succeeded = provider.parseWebhook(
      providerWebhookCtx({ Success: true, Status: "CONFIRMED", PaymentId: 123456, OrderId: "order-1", Amount: 29900 })
    );
    ...
    const failed = provider.parseWebhook(
      providerWebhookCtx({ Success: false, Status: "REJECTED", PaymentId: 123456, OrderId: "order-1" })
    );
    expect(failed).not.toBeNull();
    expect(failed!.eventType).toBe("payment.failed");
    expect(failed!.providerEventId).toBe("123456:REJECTED");

    const canceled = provider.parseWebhook(providerWebhookCtx({ Status: "CANCELED", PaymentId: 123456 }));
    ...
    const other = provider.parseWebhook(providerWebhookCtx({ Status: "FORMSHOWED", PaymentId: 123456 }));
    expect(other!.eventType).toBe("tbank.FORMSHOWED");

    // Unrecognizable payloads → null (route answers 400).
    expect(provider.parseWebhook(providerWebhookCtx({ Status: "CONFIRMED" }))).toBeNull();
    expect(provider.parseWebhook(providerWebhookCtx({ PaymentId: 123456 }))).toBeNull();
    expect(provider.parseWebhook(providerWebhookCtx({}))).toBeNull();
  });
});
