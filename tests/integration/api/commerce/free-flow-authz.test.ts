...
// harness pattern from commerce/payments-webhook-yookassa.test.ts). The
// enabled flag mirrors
...
import * as yookassaModule from "@server/lib/yookassa";
import { UPLOAD_DIR } from "@server/lib/upload";
import { createServerSigningKey, getTrustedServerKeys } from "@server/lib/drm/service";
import {
...
import type { LeasePayload } from "@server/lib/drm/types";
import { CLOCK_SKEW_SECONDS } from "@server/lib/drm/protocol";
import { resetTestEntities, createTestUser } from "@tests/tools/helpers/db-reset";
import { checkoutResource, downloadVersion } from "@tests/tools/helpers/purchase-journey";
...
async function seedVersionSignature(
  ...
  const { publicKey } = await import("@server/lib/artifact/crypto").then((m) =>
...
beforeAll(async () => {
  ...
  // licenses/drm-hardening.test.ts); the private key feeds the env the route signs with.
...
describe.skipIf(!dbAvailable)("free purchase end-to-end flow", () => {
  ...
  it("checkout completes instantly: 201 completed with a licenseId", async () => {
    const res = await checkoutResource(app, buyerToken, freeSlug);
    expect(res.status).toBe("completed");
    expect(res.purchaseId).toBeTruthy();
    expect(res.licenseId).toBeTruthy();
  });
  ...
  it("buyer downloads the artifact through the entitlement-checked route", async () => {
    const res = await downloadVersion(app, buyerToken, freeSlug, freeVersion);
    expect(res.status).toBe(200);
...
describe.skipIf(!dbAvailable)("resource authorization matrix", () => {
...
describe.skipIf(!dbAvailable)("DRM security hardening", () => {
...
describe.skipIf(!dbAvailable)("payment abuse resistance", () => {
