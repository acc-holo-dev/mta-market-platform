...
import { encryptWithDek, decryptWithDek } from "@server/lib/artifact/encryption";
import { createVersionDek } from "@server/lib/drm/service";
import { CLOCK_SKEW_SECONDS, DEK_ALGORITHM } from "@server/lib/drm/protocol";
...
beforeAll(async () => {
  ...
  const { publicKey: publisherPub } = await import("@server/lib/artifact/crypto").then((m) =>
...
describe.skipIf(!dbAvailable)("DRM protocol contract / clock skew", () => {
...
describe.skipIf(!dbAvailable)("server signing key rotation", () => {
...
describe.skipIf(!dbAvailable)("per-version DEK envelope + lease-gated release", () => {
...
describe.skipIf(!dbAvailable)("revocation policy and full cycle", () => {
  it("runs the full cycle: register -> verify -> activate -> heartbeat -> renew -> revoke -> denied", async () => {
    ...
    const { revokeInstallation } = await import("@server/lib/drm/service");
