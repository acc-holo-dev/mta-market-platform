...
import { validateResourceDependencies, versionSatisfiesMin } from "@server/lib/artifact/dependencies";
...
describe.skipIf(!dbAvailable)("seller onboarding and permission gate", () => {
...
describe.skipIf(!dbAvailable)("moderation event log", () => {
...
describe.skipIf(!dbAvailable)("dependency graph validation", () => {
  ...
  it("rejects publication when a declared dependency is missing/circular", async () => {
    const { createVersionDek } = await import("@server/lib/drm/service");
...
describe.skipIf(!dbAvailable)("compatibility report and release lifecycle", () => {
...
describe.skipIf(!dbAvailable)("marketplace review eligibility", () => {
  it("rejects a self-purchase review (seller reviewing own resource)", async () => {
    const { createResourceCheckout } = await import("@server/lib/commerce");
...
describe.skipIf(!dbAvailable)("dispute lifecycle", () => {
