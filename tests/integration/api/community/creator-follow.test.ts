...
async function bustCache(): Promise<void> {
  const keys = ["activity:live:v2"];
  for (let limit = 5; limit <= 50; limit += 5) keys.push(`activity:snapshot:v2:${limit}`);
  try {
...
describe("creator follow delivery", () => {
