...
describe.skipIf(!dbAvailable)("server review eligibility", () => {
...
describe.skipIf(!dbAvailable)("review token failure matrix", () => {
  ...
  it("wrong-server token fails and the token stays bound to its server", async () => {
    ...
    const row = await db.orm.public.ServerReviewToken
      .where({ tokenHash: (await import("@server/lib/serverIntegration")).sha256Hex(token) })
      .first();
...
describe.skipIf(!dbAvailable)("review creation & moderation", () => {
