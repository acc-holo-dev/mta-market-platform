...
import { API, RUN, uiLogin, apiLogin, ADMIN_EMAIL, ADMIN_PASSWORD } from "../../tools/playwright/helpers";
...
async function apiRegister(request: import("@playwright/test").APIRequestContext, username: string) {
  const res = await request.post(`${API}/auth/register`, {
    data: { username, email: `${username}@e2e.local`, password: PASSWORD },
  });
...
test.describe("PLAN-007 Content Foundation", () => {
  let authorToken = "";
  let articleId = "";
  let slug = "";

  test("author → moderation → Home activity → article page (§14/§17-2)", async ({ page }) => {
    authorToken = await apiRegister(page.request, AUTHOR);
  ...
  test("reject branch: the reason reaches the author (§17-3)", async ({ page }) => {
    const token = await apiRegister(page.request, `e2e_ct_rj_${RUN}`);
