...
import { API, RUN, uiLogin, apiLogin, psql, cleanupEntities } from "../../tools/playwright/fixtures";

const SEED_PASSWORD = "seed-password-123";

test.describe.configure({ mode: "serial" });

test.describe("daily experience dashboard and activity", () => {
  // News item published in the third test (authored by the seed owner).
  let createdNewsId = "";

  test.afterAll(async () => {
    // PLAN-016 D-013 hygiene: the published news row is the only entity this
    // spec creates (no e2e_* users) — remove it so the Home activity feed
    // and the server news page don't accumulate E2E rows across runs.
    try {
      await cleanupEntities({ news: createdNewsId ? [createdNewsId] : [] });
    } catch (e) {
      console.warn("[dashboard-activity afterAll] cleanup skipped:", String(e).slice(0, 200));
    }
  });
  ...
  test("owner publishes news → it appears in Home activity (§23-3)", async ({ page }) => {
    ...
    createdNewsId = (await createRes.json()).id as string;
    const newsId = createdNewsId;
