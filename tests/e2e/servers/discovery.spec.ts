...
import { RUN, uiLogin, uiRegister, cleanupEntities } from "../../tools/playwright/fixtures";
...
test.describe("community hub and forum", () => {
...
test.describe("server owner onboarding", () => {
...
test.describe("notifications and server follow", () => {
...
test.afterAll(async () => {
  // Cleanup: e2e-created server rows (slug pattern is deterministic) and the
  // owner user — canonical FK-safe order (children before the user).
  try {
    await cleanupEntities({ serverSlugPrefixes: ["e2e-server-"], usernamePrefixes: ["e2e_sv_"] });
  } catch {
    // best effort
  }
});