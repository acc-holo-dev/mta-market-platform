...
import { API, RUN, uiRegister, uiLogin, apiLogin, psql, makeArtifact } from "../../tools/playwright/helpers";
...
test.describe("PLAN-008 Follow Expansion", () => {
  ...
  test("creator publishes an article → fan gets CREATOR_ARTICLE (D-003)", async ({ page }) => {
    ...
    await page.request.post(`${API}/content/${id}/submit`, {
      headers: { Authorization: `Bearer ${sellerToken}` },
    });
    const approve = await page.request.post(`${API}/admin/content/${id}/approve`, {
      headers: { Authorization: `Bearer ${adminToken}` },
    });
    expect(approve.status()).toBe(200);
