...
test.describe("PLAN-016 shell acceptance", () => {
  // PLAN-016 D-013 hygiene: the run-created user (and its purchases) must
  // not accumulate across runs. Resources cascade with their owner; RESTRICT
  // children are deleted first. e2e-admin is never touched.
  test.afterAll(async () => {
    try {
      const ids = await psql(
        `SELECT string_agg('''' || id || '''', ',') FROM "user" WHERE username = '${SHELL_USER}'`
      );
      if (!ids) return;
      ...
      console.warn("[plan016 afterAll] cleanup skipped:", String(e).slice(0, 200));
    }
  });
